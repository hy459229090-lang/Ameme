...
| 一天上下文覆盖与长期记忆 P0 | in_progress | Goal active / repository candidate conditional_pass / Gate 1 hold | 本地覆盖、编译、删除、普通用户同安装恢复健康/切换、真实/Mock 复用与 helpful feedback、Android Agent 四操作、QR v2 一次性 bootstrap/独立凭据、iOS 四操作客户端与最终 head Xcode/Android 设备闭环已完成；等待真实 Pilot、真实 outcome/Host、规模与跨设备生产恢复；`JOBS-Ameme.md#一天上下文覆盖与长期记忆-p0` |
...
| 封闭 Beta 候选持续执行 | in_progress | production P0 / external gates hold | Coverage、事件、长期 Memory、普通用户同安装恢复点/真实健康/exact 切换、真实/Mock 复用、来源/space 删除、字段证据、用户确认 provenance、Android Agent bounded read/write/undo、QR v2 服务端消费/独立凭据、Android 系统扫码/显式粘贴 client 和 iOS device-only 恢复已接生产并获最终 head CI；物理扫码/无 Play 真机、真实用户/设备/读屏/共享 Grant/跨设备恢复/签名/商店 Gate 继续 `hold`；`JOBS-Ameme.md#封闭-beta-候选持续执行-goal` |
