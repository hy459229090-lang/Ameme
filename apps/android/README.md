...
- Settings now presents one ordinary-user “设备连接” flow with three entrances: same-LAN discovery, QR scan, and account devices. In Debug, all three use a deterministic connector that resolves the same bounded synthetic candidate, requires an explicit authorization screen, persists only non-sensitive connection metadata, and clearly says that no real network connection or data transfer occurs. Release has no synthetic connector. The existing manual TLS secret/JSON flow is retained only under Debug developer options for real Host-channel engineering verification.
