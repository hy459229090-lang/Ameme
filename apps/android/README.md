...
The merged device baseline is 34 ordinary API 36 AVD tests plus one separately gated DB-01 performance test. Calendar/voice integration, provider denial/cancellation, all-day identity, repository lifecycle, and the 501-event batch fast path are covered on that AVD. Physical-device checks remain required for OEM calendar providers, recurring/all-day/timezone behavior, system recorder availability/result grants, process recreation, permission revocation, 16 KB page-size readiness, UI startup/paging, backup/restore, physical deletion, Raw Vault, and peer sync. FTS5 is verified only for the tested SQLCipher runtime; other runtime/device combinations may use the tested LIKE fallback.
