...
- 设置提供普通用户“同安装恢复”：可创建/更新有界恢复点，只有实际恢复并打开隔离候选后才显示“已验证”，展示创建、最近验证和最近成功恢复时间；切换要求逐字输入 `恢复`。恢复点位于 live store 之外但仍只属于当前安装，依赖 `ThisDeviceOnly` Keychain 密钥，卸载、换机或设备丢失后不可用，也不宣称为云备份或生产灾备
- 设置中的来源边界、隐私说明和 Agent 三入口连接体验；同一局域网入口调用 `_ameme-agent._tcp` 的 Bonjour/Network.framework 发现并展示授权前候选，二维码入口可扫描 Android 端 5 分钟 v2 bootstrap，在 TLS pin 下提交 P-256 私钥持有证明并换取独立 30 天 channel credential，再建立冻结 v1 TLS 1.3/HMAC 应用通道；账户设备未接入时明确失败，只有显式“试用演示连接（不联网）”才保存模拟状态，真实发现不会自动授权
- Agent 三入口统一保存为受限的非敏感体验连接状态（设备、Agent、能力、方式、30 天期限和模拟标记），损坏或过期状态自动断开；密钥、端点、证书 pin、正文和真实网络连接不进入该状态
- `AgentAccessGrant`/`AgentAccessGrantPolicy` 与 canonical AccessGrant 对齐 caller/Grant 绑定、purpose、Personal space、structured event、期限和撤销语义；`AgentLocalNodeChannelCodec` 与 Android/Python golden 逐字一致，`AgentLocalNodeNetworkClient` 提供 TLS 1.3、证书 pin、HMAC session/frame、序列/nonce 校验，以及四项 Android 生产 v1 operation 的最小 Grant builder、握手 capability 检查和 typed response/result-digest/error-shape fail-closed。原始 application exchange 不对外开放，生产 exchange 只按当前时间授权。普通用户 QR policy 与设置授权卡片仍只授权/显示结构化 Event 写入，不因新增 API 静默获得读取、Revision 或撤销；明确 expanded Grant 下才可调用 `append_revision`、exact `undo_capture` 与 bounded `visible_events`。Shared Smoke 与 XCTest 源码验证协议、最小 scope、policy 绑定、过期、撤销和恶意响应；共享 Grant registry、真实 advertiser/host 和物理设备级传输仍待补
- 配对材料解析先经过严格 JSON grammar/duplicate-key/canonical 检查，再进入 Foundation；v1 QR fail closed。iOS 会在请求前把 pending envelope、P-256 私钥与 thumbprint 保存到 `kSecAttrAccessibleWhenUnlockedThisDeviceOnly` Keychain，响应丢失或进程重启后仅用同一 key 重试；签发成功后保存独立 credential，并在实际重新完成 TLS/HMAC 认证后才恢复“已连接”。断开和本机 Space 删除会关闭 client/channel 并清除 Keychain 与展示状态。该 key binding 约束签发与恢复，冻结应用通道没有每次重连 P-256 proof；账户/共享 Grant registry 仍是后续门禁
...
- `AmemeSharedSmoke` 可在当前 macOS Command Line Tools 环境验证本机记录、空白/稀疏/正常状态、关键词/范围查询面、Revision、小结、结构化导出恢复快照（密文往返/清理）、删除、批量导入回滚、演示数据隔离、opaque incoming-share handoff，Coverage 密文保存/重载、显式 Candidate 接受、source cascade/link detach/source watermark 和重载后不复活，exact-revision 多来源字段证据/重算/重载、完整用户确认删源保留且去除来源声明、partial 确认 fail-closed 删除与加密重载，长期 Memory 候选/显式确认/Event revision 失效/密文重载不复活，四类复用的原子 exact-content resolution、Restricted 排除与无正文反馈，app-owned media Raw-only 与外部原件边界，认证同安装备份、过期激活授权零变更、exact-confirmation 激活/删除不复活/候选不消费、受控恢复点创建/实际候选验证/切换/重启 receipt，当前安装 Personal space 根冻结、app-owned Raw cleanup、删除后旧快照精确拒绝和重载防复活，以及 QR v2 golden、signed bootstrap tamper rejection、device-only Keychain pending→active→expiry/clear；它不替代 XCTest 中途故障注入、SwiftUI/XCUITest、用户可见生产恢复、真实用户确认动作、真实 helpful outcome、account/Grant/peer deletion、跨设备 key recovery 或真机验收
- `AmemeLocalNodeSmoke` 已在 API 36 / 16 KB arm64 AVD 实际执行 Swift Network.framework client → Android Local Node 的 QR v2 bootstrap、bootstrap/application credential separation、签发时 client-key binding，再以独立 credential 完成 TLS 1.3、certificate pin、HMAC session/frame、Grant-bound create→bounded read→Revision→Revision undo→Event undo→final read，并保留一个原始 Event 供 Today 复核。这是 ADB-forward 的虚拟设备纵向子门，不等于真实相机光学扫码、物理 LAN、iOS/Android 真机、真实 Host/ContextPack 或发布通过
...
仓库的 iOS GitHub Actions 会在 Xcode 26.6 / iOS 26.5 Simulator 上分别执行浅色默认大字体的完整测试，以及深色无障碍超大字体的 UI 测试；两组运行都把截图保存在独立 `.xcresult` 并导出为可直接复核的 artifact 附件。实现/证据 head `8291ced` 的 iOS CI `31246698219` 已通过 App + embedded Share Extension、71 Unit、普通字号 Mock/真实本机 UI 2/2、深色 XXXL UI 1/1，并导出 15 张截图。本地环境若只有 Command Line Tools，仍不能把 Shared Smoke 当作完整 Xcode、Simulator、Liquid Glass 渲染、签名或真机证据；Simulator 截图也不替代真实 VoiceOver。
...
本地事件、来源定位和用户补充只在主动操作后写入本机。系统权限仅在用户实际点击照片、录音、日历或文件导入时请求。当前 iOS 端可通过 Android 短时 QR v2 bootstrap 换取独立设备专属存储的 credential，并建立 Local Node client 通道；真实相机/物理 LAN 仍需双端真机签收，账户设备 registry、共享 Grant 撤销传播、HealthKit 适配器和 AI 网关也未接入，设置页不会伪造连接成功。
