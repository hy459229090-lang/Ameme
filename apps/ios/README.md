...
- `AmemeLocalNodeSmoke` 已在 API 36 / 16 KB arm64 AVD 实际执行 Swift Network.framework client → Android Local Node 的 TLS 1.3、certificate pin、HMAC session/frame、Grant-bound create→bounded read→Revision→Revision undo→Event undo→final read，并保留一个原始 Event 供 Today 复核。这是 ADB-forward 的模拟器纵向子门，不等于物理 LAN、iOS Simulator/真机、真实 Host/ContextPack 或发布通过
