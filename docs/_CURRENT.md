...
上段 Android 100/93/7/0 是 QR v2 接入前的设备基线；其后的 QR v2 切片为 Debug/Release JVM 各 110/110、API 36 / 16 KB AVD 103/96/7/0、QR manager 7/7、QR 静态 Gate 54/54。同安装恢复切片进一步更新为 Debug JVM 112/112、AVD 105/98/7/0、恢复 6/6、完整 UI 14/14；随后 Android 普通用户 QR client 和显式粘贴替代把当前结果更新为 Debug/Release JVM 各 121/121、AVD 115/108/7/0、QR UI 6/6、QR 静态 Gate 62/62；APK 为 Debug 43,811,576 bytes、unsigned Release 35,763,367 bytes、androidTest 2,847,128 bytes。iOS Shared/App/SharedSmoke/LocalNodeSmoke 与 device-only Keychain Smoke 通过；实现已收口到 `8d04bc7` 并推送 PR，最终 head 的 Android、iOS 与 workspace CI 仍须全部签收。

2026-07-29 的本机删除入口进一步把 root freeze 与当前安装收敛串联：设置页要求逐字输入 `删除`，随后停止并等待本机 Agent runtime、验证移除当前安装 pairing/secret 与连接元数据，并清空 Android pending action、iOS pending export/App Group incoming-share handoff，写 content-free marker 阻止重启复活。Android JVM 107/107、API 36 / 16 KB AVD 100/93/7/0、iOS 生产 Smoke 与跨端静态契约 206 项通过；iOS XCTest 尚未在最终 head 执行。该结论是该删除切片的历史执行点，只覆盖当前安装，不代表账号或共享 Grant registry、peer/云副本、provider 原件、外部分享、物理擦除或物理设备执行。

同日 QR 安全复核先确认 v1 的 5 分钟 parser/UI 过期不构成服务端不可重放证明，随后仓库实现切换到 `ameme.agent-pairing-bootstrap.v2`：Android 原子消费短时 bootstrap、验证 P-256 client-key possession、签发独立 30 天 channel credential，并只允许同 key 有界重取；Release 不再持有 developer bearer。iOS 将 pending/active credential 与 P-256 key 保存到 device-only Keychain，启动后只有实际重新完成 TLS/HMAC 认证才恢复连接。后续 Android Release 已接用户主动系统 QR-only scanner 与显式粘贴完整码替代；后者不读取剪贴板、输入 16,384 字符有界且取消/提交清除，两条入口均复用 event-only 确认与同一认证链。QR 静态 Gate 62/62、API 36 / 16 KB AVD 115/108/7/0、Swift→Android v2 AVD smoke 和 Debug Host 独立凭据路径通过。该 key binding 只精确声明签发/恢复绑定，不声明冻结应用通道每次重连都有 P-256 proof；共享 Grant registry、最终 head Xcode/XCTest、物理扫码、无 Play 真机与物理设备仍为 `hold`。

同日普通用户同安装恢复切片进一步关闭了产品入口空白：双端设置页可创建/更新 current/previous 有界恢复点，只有真实恢复并打开隔离候选后才显示健康，展示创建/验证/最近成功恢复状态，并要求逐字输入 `恢复`。Android 激活前停止 Agent、关闭 SQLCipher，成功或回退后重开并重载 Today；iOS 恢复点位于 live `Application Support/Ameme` 同级受控根。Android Debug JVM 112/112、API 36 / 16 KB arm64 AVD 105 discovered / 98 passed / 7 外部门 skipped / 0 failed、完整 UI 14/14；iOS Debug/Release build 与生产 Smoke、跨端静态 Gate 210/210 通过。本机 `swift test` 仍真实失败于缺少 `XCTest`。该点依赖当前安装的 Keystore/Keychain key，卸载、换机或设备丢失后不可用，`productionRecoveryClaim=false`；跨设备生产恢复、最终 head Xcode、物理设备和真实用户继续 `hold`。
...
| P0 双端普通用户同安装恢复与健康状态 | `quality/P0-双端普通用户同安装恢复与健康状态验证-20260729.md` | conditional_pass；双端设置页提供有界恢复点、实际隔离恢复健康、最近成功与逐字 `恢复`；Android JVM 112/112、API 36 / 16 KB AVD 105/98/7/0 与 UI 14/14，iOS Debug/Release Smoke、静态 Gate 210 通过；跨设备 key、最终 head Xcode、物理恢复和真实用户 hold |
...
| P0 QR 一次性 Bootstrap 与设备凭据轮换 | `quality/P0-QR一次性Bootstrap与设备凭据轮换验证-20260729.md` | conditional_pass；服务端消费、签发时 P-256 key binding、独立 credential、Release 隔离与设备专属恢复已实现；当时记录的 Android scanner client 缺口已由下一项补齐，共享 Grant、最终 head XCTest、物理扫码/LAN hold |
