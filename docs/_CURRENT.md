...
| MVP 本地存储、同步与删除 | `architecture/MVP本地存储同步与删除协议.md` | v0.20，Android SQLCipher 4.15.0/v13、iOS envelope v8、space/Revision/SourceLocator/Coverage/长期 Memory/删除水位/恢复候选与同安装激活回滚/复用/来源证据/用户确认 provenance/space root、Android Agent bounded read/write/exact undo、iOS 四操作 canonical/typed 客户端与 AVD 纵向网络闭环；用户恢复 UI、iOS XCTest、真实用户确认/ContextPack、account/Grant/peer proof、跨设备 key、完整 Raw/LAN 仍待验证 |
