...
上段 Android 100/93/7/0 是 QR v2 接入前的设备基线；其后的 QR v2 切片为 Debug/Release JVM 各 110/110、API 36 / 16 KB AVD 103/96/7/0、QR manager 7/7、QR 静态 Gate 54/54。最新同安装恢复切片已进一步更新为 Debug JVM 112/112、AVD 105/98/7/0、恢复 6/6、完整 UI 14/14；APK 为 Debug 41,261,185 bytes、unsigned Release 33,690,590 bytes、androidTest 2,943,673 bytes。iOS Shared/App/SharedSmoke/LocalNodeSmoke 与 device-only Keychain Smoke 通过，但新增 bootstrap 与恢复管理器 XCTest 尚未在最终 head 的完整 Xcode 执行。
...
同日 QR 安全复核先确认 v1 的 5 分钟 parser/UI 过期不构成服务端不可重放证明，随后仓库实现切换到 `ameme.agent-pairing-bootstrap.v2`：Android 原子消费短时 bootstrap、验证 P-256 client-key possession、签发独立 30 天 channel credential，并只允许同 key 有界重取；Release 不再持有 developer bearer。iOS 将 pending/active credential 与 P-256 key 保存到 device-only Keychain，启动后只有实际重新完成 TLS/HMAC 认证才恢复连接。静态 Gate 54/54、Android manager AVD 7/7、Swift→Android v2 AVD smoke 和 Debug Host 独立凭据路径通过。该 key binding 只精确声明签发/恢复绑定，不声明冻结应用通道每次重连都有 P-256 proof；Android 相机扫码生产 client、共享 Grant registry、最终 head Xcode/XCTest 与物理设备仍为 `hold`。
