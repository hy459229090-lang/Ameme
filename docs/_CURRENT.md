...
上段 Android 100/93/7/0 是 QR v2 接入前的设备基线；当前最终仓库子门已更新为 Debug/Release JVM 各 110/110、API 36 / 16 KB AVD 103 discovered / 96 passed / 7 外部门 skipped / 0 failed、QR manager 7/7、QR 静态 Gate 54/54、workspace 28/28。当前 APK 为 Debug 41,190,345 bytes、unsigned Release 33,592,286 bytes、androidTest 2,874,573 bytes。Android 设置页已把连接信息拆为三段 Material 3 原生卡片，普通 QR 默认收起 Debug 开发者材料；Today、设置与 QR 均完成 130% 字号截图复核。iOS Shared/App/SharedSmoke/LocalNodeSmoke 与 device-only Keychain Smoke 通过，但新增 bootstrap XCTest 尚未在最终 head 的完整 Xcode 执行。
