...
Scope / Build / Commit: Android Release 普通用户 QR v2 系统扫码、设备密钥、TLS 签发、应用通道认证与同安装重连；未提交工作区
...
不匹配或应用认证失败时均不建立连接，并显示可理解状态。当前没有为无 Play services 设备
提供手工粘贴替代，这仍是兼容性 Gate。
...
设备为 API 36 arm64 AVD，`getconf PAGE_SIZE=16384`，1080×2400，字号 1.3。完整 XML 精确为
112 discovered / 105 passed / 7 个显式外部门 skipped / 0 failed。
...
| 旧 HEAD `1acc1af` Android CI 的 Event 详情 UI 用例找不到刚保存的动态标题 | 用例只等待保存弹窗消失，没有等待异步写入后的 Event 进入 LazyColumn 语义树 | 当前工作树在点击前对 `today-list` 执行精确 `performScrollToNode` 并断言显示；同一用例本地为 3.956s，完整 112/105/7/0 通过 | 新 HEAD CI 需在获得发布授权后签收 |
...
- 无 Google Play services 的设备兼容路线和手工输入无障碍替代；
...
`P0-ANDROID-QR-V2-SCANNER-CLIENT-07` 关闭了仓库内 Android Release “普通用户无法扫描并建立
经过认证的 QR v2 连接”的可执行空白，并由完整 AVD 回归捕获及修复了 TLS alias 跨域选择
缺陷。结论保持 `conditional_pass`：仓库候选可复跑，但物理扫码、真实 LAN、真实用户和发布
Gate 仍为 `hold`。
