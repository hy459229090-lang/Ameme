...
MVP 工程实现进行中。已集成契约质量基线、非生产 Core/Raw/队列 Oracle、同步协议模拟器、Agent MCP Mock↔Core 测试适配、R0 AI/12-case 固定评测与 Android SQLCipher 加密 Local Event Node；Android 后续切片在 API 36 AVD 通过 25 项设备测试，覆盖 v4 SourceLocator、持久 URI 两阶段清理、Photo Picker/ACTION_SEND 边界、Calendar planned 适配器、FTS5/LIKE 多词共同语义和 keyset 日期分页。生产 Raw/LAN、Calendar Provider/语音、真实模型/宿主、双端真机、16 KB、异步 I/O 和性能容量仍待实现或验证。
