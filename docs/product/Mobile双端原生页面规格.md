...
```text
Settings -> 设备连接
 -> 选择“自动发现电脑 / 扫描二维码 / 账户设备”之一
 -> 找到候选设备
 -> Mobile 统一显示设备、Agent 身份、用途、空间、数据类型和有效期
 -> 用户允许或收窄范围
 -> 生成可撤销设备/Agent grant
 -> 每次调用进入访问记录
```

三种入口是不同的发现方式，不是三套授权模型。普通用户页面不得要求复制密钥、JSON、IP 或端口；连接后显示设备名、Agent、连接方式、授权范围、最近使用和断开入口。配对挑战短时有效、一次使用；不能在二维码/代码中包含长期密钥。具体协议、TTL 和设备证明由安全/架构正本冻结。

Android Debug 可以用明确标注的体验连接验证完整交互，但必须满足：不建立真实网络、不写 Event、不伪造审计、状态可断开、Release 无体验 Connector。真实手工 TLS 配对仅在 Debug 开发者选项可见。iOS 在实现相同语义时使用原生 sheet、扫码器和发现/账户选择组件，不复制 Android 布局。
