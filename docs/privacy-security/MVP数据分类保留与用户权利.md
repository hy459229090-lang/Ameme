...
同日补齐的 `deleteLocalSpace` 只执行当前安装 Personal space 的根冻结与本机收敛：先终结可读 Event/Coverage/长期 Memory/复用/source 投影，最后写不可被旧备份绕过的 SPACE 水位；重载后继续拒绝普通读写。iOS 删除 app-owned Raw ciphertext，Android 继续处理 persisted locator release，但两端都保留无正文 tombstone/删除记录用于防复活与审计。该结果硬编码不宣称账号删除或 peer proof，也不撤 Grant/Contract、不删除 Photos/provider 原件、对端/云副本或外部分享，不证明物理介质擦除。只有这些分布式步骤和用户可见权利请求流程完成后，才可把第 5 项“删除 space/account”报告为全局完成。
